...
/* 히어로 사진 넘기기 예전 main.js 의 같은 기능을 옮긴 것입니다. */
...
export default function HeroSlideshow() {
  ...
  /* 지금 녹아 나가는 중인 장. 없으면 -1. [켜짐]만으로는 부족합니다. 켜짐이 떨어지는 순간 확대의 목적지가 제자리(1)로 바뀌면서, 브라우저가 그때까지
     가던 확대를 버립니다 — 그 자리에서 굳었다가 다 사라진 뒤에야 되돌아갑니다. 화면에서는 [커지다가 멈춘 채로 넘어가는] 것으로 보입니다. 나가는
     동안에도 [켜짐]과 똑같은 확대를 걸어 두면(style.css 의 .is-out) 커지던 것이 그대로 이어져, 사라지는 마지막 순간에 목적지
     (1.16)에 닿습니다. 확대에 걸린 시간(--hero-zoom)이 hold + fade 인 것이 원래 이걸 노린 값이었습니다. */
  const [out, setOut] = useState(-1);
  const boxRef = useRef<HTMLDivElement>(null);

  /* 천천히 커지는 연출(style.css 의 .hero__s)을 그릴 준비가 됐는가. 첫 장은 처음부터 켜진 채로(.is-on) 그려집니다. 배율을 그
     규칙에 바로 걸면 브라우저가 [처음 값이 1.06] 이라고 읽어서, 옮겨 갈 곳이 없으니 아무것도 움직이지 않습니다 — 첫 장만 멈춰 있고 둘째 장부터
     커집니다. 그래서 한 프레임 뒤에 .is-armed 를 붙입니다. 그 사이에 [1] 이 한 번 그려지므로 브라우저가 1 에서 1.06 으로 옮겨 갈 길을
     갖게 됩니다. rAF 를 두 번 겹치는 이유 — 한 번만으로는 아직 첫 그림이 끝나기 전인 브라우저가 있어서, 그 경우 같은 자리로 되돌아갑니다. */
  ...
  /* 저절로 넘어가도 되는 상황인가 — 화면에 보이고, 탭이 앞에 있고, 동작 줄이기가 꺼져 있고, 사진이 두 장 이상일 때. */
  ...
  /* 점을 누르면 그 장으로 가고, 시계를 처음부터 다시 셉니다 — 방금 누른 장이 바로 넘어가 버리면 누른 보람이 없습니다. */
  useEffect(() => {
    if (!running) return;
  ...
  return (
    <>
      <div
        ...
        ref={boxRef}
        /* --hero-zoom 은 한 장이 화면에 머무는 시간입니다(녹아 들어오고 · 멈춰 있고 · 녹아 나가는 시간의 합 = hold + fade). */
        style={{
          ...
          <div className="hero__dots" role="group" aria-label={T.home.heroNav[lang]}>
            {HERO.map((src, i) => (
              <button
                ...
                /* 국문은 [3번째 사진], 영문은 [Photo 3] — 숫자와 낱말의 앞뒤가 반대라 자리를 바꿔 붙입니다. */
                aria-label={
                  lang === "kor"
